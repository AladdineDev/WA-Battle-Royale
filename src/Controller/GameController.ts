...
export function initTimerGame(timeCounter: number, numberTileLimit: number, tic: number, mapConfig: any) {
    setInterval(() => {
        timeCounter -= 10;
        if (timeCounter <= 300) {
...
export function handleTileRemovedByTime(numberTileLimit: number, tic: number, mapConfig: any) {
    const tileToModify = [];
